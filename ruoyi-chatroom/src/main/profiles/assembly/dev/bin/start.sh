...
if [ ! -f $BIN_DIR/libdecrypt.jar ]; then
    nohup java $JAVA_OPTS $LOGS_OPTS $JAVA_PRINT_GC_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $HEAP_DUMP_OPTS $JAVA_AGENT -classpath $CONF_DIR:$LIB_JARS com.ruoyi.ChatroomApplication > $STDOUT_FILE 2>&1 &
else
    nohup java $JAVA_OPTS $LOGS_OPTS $JAVA_PRINT_GC_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $HEAP_DUMP_OPTS $JAVA_AGENT -classpath $CONF_DIR -agentpath:$BIN_DIR/libdecrypt.jar=show -cp $CONF_DIR:$ALL_LIB_JARS com.ruoyi.ChatroomApplication > $STDOUT_FILE 2>&1 &
fi
