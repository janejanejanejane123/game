...
public class SysJobController extends BaseController
{
    ...
    /**
     * 新增定时任务(涉及到安全，只对超级管理员开放，且只能调用bean执行,格式为:类名.方法名)
     */
    ...
    public AjaxResult add(@RequestBody SysJob job) throws SchedulerException, TaskException
    {
        ...
        if(!isAdmin){
            return AjaxResult.error(HttpStatus.NOT_SUPERADMIN, "仅对超级管理员开放!");
        }
        ...
        if(invokeTarget.indexOf(".") < 1){
            return error("新增任务'" + job.getJobName() + "'失败，只支持格式为:类名.方法名的BEAN方式调用");
        }
        ...
        Object bean = SpringContextUtil.getBean(invokeTarget.split("\\.")[0]);
        if(bean == null || !bean.getClass().getPackage().getName().startsWith("com.ruoyi.quartz.task")){
            return error("新增任务'" + job.getJobName() + "'失败，未自定义此bean!");
        }
    ...
    public AjaxResult edit(@RequestBody SysJob job) throws SchedulerException, TaskException
    {
        ...
        String invokeTarget = job.getInvokeTarget();
        if(invokeTarget.indexOf(".") < 1){
            return error("修改任务'" + job.getJobName() + "'失败，只支持格式为:类名.方法名的BEAN方式调用");
        }
        ...
        Object bean = SpringContextUtil.getBean(invokeTarget.split("\\.")[0]);
        if(bean == null || !bean.getClass().getPackage().getName().startsWith("com.ruoyi.quartz.task")){
            return error("修改任务'" + job.getJobName() + "'失败，未自定义此bean!");
        }
