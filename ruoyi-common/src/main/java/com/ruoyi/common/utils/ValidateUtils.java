...
public class ValidateUtils {
    ...
    public static void validateEntity(Object object, Class<?>... groups) {
        ...
        if (!constraintViolations.isEmpty()) {
            ...
            throw new ServiceException(MessageUtils.message(messageCode));
        }
    }
     ...
     * @param groups 待校验的组
     */
    public static void validateEntity(Object object,int errorCode, Class<?>... groups) {
        Set<ConstraintViolation<Object>> constraintViolations = validator.validate(object, groups);
        if (!constraintViolations.isEmpty()) {
            ConstraintViolation<Object> constraint = (ConstraintViolation<Object>) constraintViolations.iterator().next();
            String messageCode = constraint.getMessage();
            throw new ServiceException(messageCode,errorCode);
        }
    }
...
//		Pattern pattern = Pattern.compile("^[\\u4E00-\\u9FA5]{0,16}+(·[\\u4E00-\\u9FA5]{0,16}+)*$");
        Pattern pattern = Pattern.compile("^[\\u4E00-\\u9FA5][•·\\u4e00-\\u9fa5]{1,20}");
        return pattern.matcher(str).matches();
    }
