...
public class GenericsUtils {
     ...
     * @return
     */
    public static List<Type> getSuperClassGenric(Class clazz){
        List<Type> result = null;
        Type[] interfacesTypes = clazz.getGenericInterfaces();
        for (Type genType : interfacesTypes) {
            if (!(genType instanceof ParameterizedType)) {
                continue;
            }
            Type[] genericType2 = ((ParameterizedType) genType).getActualTypeArguments();
            if(genericType2 != null) {
                result = new ArrayList<>(genericType2.length);
                for (Type t2 : genericType2) {
                    result.add(t2);
                }
            }
        }
        return result == null ? new ArrayList<Type>(0) : result;
    }
